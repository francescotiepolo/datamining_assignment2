...
print(exp.model_fairness(X_train["srch_children_count"].apply(lambda x: "0" if x == 0 else "1"), "0").fairness_check(epsilon=0.8))
print(exp.model_fairness(X_train["srch_adults_count"].apply(lambda x: "1" if x == 1 else ("2" if x == 2 else "3")), "1").fairness_check(epsilon=0.8))
print(exp.model_fairness(X_train["srch_adults_count"].apply(lambda x: "1" if x == 1 else ("2" if x == 2 else "3")), "2").fairness_check(epsilon=0.8))
print(exp.model_fairness(X_train["srch_booking_window"].apply(lambda x: "short" if x <= 30 else "long"), "long").fairness_check(epsilon=0.8))
print(exp.model_fairness(X_train["srch_length_of_stay"].apply(lambda x: "short" if x <= 6 else "long"), "long").fairness_check(epsilon=0.8))
quantile = X_train["price_usd"].quantile(0.75)
print(exp.model_fairness(X_train["price_usd"].apply(lambda x: "cheap" if x <= quantile else "expensive"), "expensive").fairness_check(epsilon=0.8))
print(exp.model_fairness(X_train["srch_saturday_night_bool"].apply(lambda x: "weekday" if x == 1 else "weekend"), "weekday").fairness_check(epsilon=0.8))